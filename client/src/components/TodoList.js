...
function TodoList() {
  ...
  const addTodo = async todo => {
    ...
    const {data: {todo: newTodo}} = await axios.post('/', {text: todo.text})
    // update list
  ...
  const updateTodo = async (todoId, todoForUpdate) => {
    ...
    const {data: {updatedTodo}} = await axios.patch(`/${todoId}`, {text: todoForUpdate.text})
    // update the list
    setTodos(todos =>
  ...
  const completeTodo = async (id, isComplete) => {
    // update with the backend
    const {data: {updatedTodo}} = await axios.patch(`/${id}`, {isComplete: !isComplete})
    // update the list
    setTodos(todos =>
